...
def forget(request):
    if request.method == 'POST':
        ...
        try:
            ...
            # Perform the necessary steps to reset the password (e.g., sending a password reset email)
            # ...
...
#         user = Userprofile.objects.get(id=id)
#         fname = user.firstname  # Retrieve the first name from the user object
#         post = Posts(content=content,image=image, video=video,audio=audio ,user=user)
#         post.save()
#         return redirect(reverse('profile', args=[id]))
#     else:
#         user = Userprofile.objects.get(id=id)
...
def myads(request, id):    
    try:
        ...
        remaining_validity = []
        for ad in ads:
            fetching_date = ad.date
            validity_period = timedelta(days=30)
            expiry_date = fetching_date + validity_period
            remaining_validity = (expiry_date - fetching_date).days
            ad.remaining_validity = remaining_validity
            if remaining_validity <= 0:
                ad.delete()    
        return render(request, 'myads.html', {'ads':ads,'product':product,'fname': fname,'remaining_validity':remaining_validity})
    except ObjectDoesNotExist:
